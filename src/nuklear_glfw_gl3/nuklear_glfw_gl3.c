/*
 * Nuklear library: nuklear_glfw_gl3.c
 *
...
{
   struct nk_glfw* glfw = hb_parptr( 1 );
   GLFWwindow *window = hb_window_Param( 2 );

   if( glfw && window && hb_param( 3, HB_IT_INTEGER ) != NULL )
   {
      hb_retptr( nk_glfw3_init( glfw, window, hb_parni( 3 ) ) );
   }
   else
   {
      HB_ERR_ARGS();
...
HB_FUNC( NK_GLFW3_FONT_STASH_BEGIN )
{
   ...
   if( glfw && hb_param( 2, HB_IT_NIL ) == NULL )
   {
...
HB_FUNC( NK_GLFW3_RENDER )
{
   ...
   if( glfw && hb_param( 2, HB_IT_INTEGER ) != NULL && hb_param( 3, HB_IT_INTEGER ) != NULL && hb_param( 4, HB_IT_INTEGER ) != NULL )
   {
      nk_glfw3_render( glfw, hb_parni( 2 ), hb_parni( 3 ), hb_parni( 4 ) );
   }
   else
   {
      HB_ERR_ARGS();
...
HB_FUNC( NK_GLFW3_DEVICE_DESTROY )
{
   struct nk_glfw* glfw = hb_parptr( 1 );
...
HB_FUNC( NK_GLFW3_DEVICE_CREATE )
{
   struct nk_glfw* glfw = hb_parptr( 1 );
...
HB_FUNC( NK_GLFW3_CHAR_CALLBACK )
{
   GLFWwindow *window = hb_window_Param( 1 );

   if( window && hb_param( 2, HB_IT_INTEGER ) != NULL )
   {
      nk_glfw3_char_callback( window, ( unsigned int ) hb_parni( 2 ) );
   }
   else
   {
      HB_ERR_ARGS();
...
HB_FUNC( NK_GFLW3_SCROLL_CALLBACK )
{
   GLFWwindow *window = hb_window_Param( 1 );

   if( window && hb_param( 2, HB_IT_NUMERIC ) != NULL && hb_param( 3, HB_IT_NUMERIC ) != NULL )
   {
      nk_gflw3_scroll_callback( window, hb_parnd( 2 ), hb_parnd( 3 ) );
   }
   else
   {
      HB_ERR_ARGS();
...
HB_FUNC( NK_GLFW3_MOUSE_BUTTON_CALLBACK )
{
   GLFWwindow *window = hb_window_Param( 1 );

   if( window && hb_param( 2, HB_IT_INTEGER ) != NULL && hb_param( 3, HB_IT_INTEGER ) != NULL && hb_param( 4, HB_IT_INTEGER ) != NULL )
   {
      nk_glfw3_mouse_button_callback( window, hb_parni( 2 ), hb_parni( 3 ), hb_parni( 4 ) );
   }
   else
   {
      HB_ERR_ARGS();
